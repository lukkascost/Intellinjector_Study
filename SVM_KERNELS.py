...
oDataSet = DataSet()
base = np.loadtxt("ATTS/INJ_05.txt", usecols=range(9), delimiter=",")
classes = np.loadtxt("ATTS/INJ_05.txt", dtype=object, usecols=9, delimiter=",")
...
for j in range(50):
    ...
    oData.random_training_test_by_percent([64, 63, 57], 0.8)
    svm = cv2.SVM()
...
oExp.add_data_set(oDataSet, description="  50 execucoes SVM_{} base Injetoras arquivos em INJ_05 sem Normalizacao. ".format(KERNEL))
oExp.save("Objects/INJ_05_CL-01-02-03_SVM_{}_MOLD-{:02d}_.gzip".format(KERNEL,MOLD))

oExp = oExp.load("Objects/INJ_05_CL-01-02-03_SVM_{}_MOLD-{:02d}_.gzip".format(KERNEL,MOLD))
